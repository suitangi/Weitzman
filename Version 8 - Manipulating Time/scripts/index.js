...
function roundBetter(num, place) {
  ...
  return (
    Math.round(
      num * mod +
        Math.sign(num) *
          0.1 ** (17 - 2 - (Math.round(num * mod) / mod).toString().length)
    ) / mod
  );
}
    ...
    if (question.type == "textbox") {
      ...
      for (let i = 0; i < question.choices.length; i++) {
        if (question.choices[i].toLowerCase() != "other") {
          ...
          html +=
            '<label class="radioContainer" onclick="otherRadioClick()"><input type="radio" name="radio"><label>Other: <input type="text" class="radioOther"></label><span class="checkmark"></span> </label>';
        }
      }
    ...
    window.preModal = $.confirm({
          ...
          action: function () {
            if (question.type == "textbox") {
              ...
              for (let i = 0; i < choiceList.length; i++) {
                exCheck.push(i + 1);
                radioList =
                  choiceList[i].getElementsByClassName("radioContainer");
              ...
              if (exCheck.length == 0) {
                ...
                $.alert({
                  ...
                  useBootstrap: false,
                  content:
                    "The following questions were incorrect: <br><strong>" +
                    exCheck.toString() +
                    "</strong><br>Please make sure to read carefully and answer them correctly.<br>",
                  type: "red",
                });
                return false;
              }
              ...
              for (let j = 0; j < radioList.length; j++) {
                if (radioList[j].getElementsByTagName("input")[0].checked) {
                  if (question.choices[j].toLowerCase() != "other") {
                    ...
                    this.$content.find(".radioOther").val().length == 0
                  ) {
                    $.alert({
                      title: "Error",
                      boxWidth: "25%",
                      useBootstrap: false,
                      content:
                        "If you select other, please do not leave it blank.",
                      type: "red",
                    });
                    return false;
                  } else {
              ...
              for (let j = 0; j < radioList.length; j++) {
                if (radioList[j].getElementsByTagName("input")[0].checked) {
                  ...
                  if (
                    question.choices[j] == question.choices[question.correct]
                  ) {
                    //correcly answered
                    preQuestions(qNum + 1);
                    ...
                    $.confirm({
                      title: "Error",
                      content:
                        "You did not answer the comprehension question correctly. Please return your submission on Prolific by selecting the 'Stop without completing' button.",
                      type: "red",
                      boxWidth: "55%",
                      useBootstrap: false,
                      typeAnimated: true,
      ...
      onContentReady: function () {
        ...
        if (question.scroll !== null && question.scroll == true) {
          if (
            $(".jconfirm-content-pane").scrollTop() +
              $(".jconfirm-content-pane").height() >=
            $(".jconfirm-content").height() * 0.95
          ) {
...
function postQuestions(qNum) {
  if (qNum == window.expParam.postquestions.length) {
    ...
    saveData(
      new Date().getTime() + "" + Math.floor(Math.random() * 10) + ".csv",
      dataToCSV()
    );
  } else {
    ...
    if (question.type == "textbox") {
      ...
      for (let i = 0; i < question.boxes.length; i++) {
        html +=
          '<tr><td style="border: 1px solid #000; padding: 10 50; text-align: center;">' +
          question.boxes[i] +
          "</td>";
        html +=
          '<td style="padding-left: 50px;"><input style="font-size: 17px; width: 200px;" type="number" placeholder="Textbox" id="boxnum' +
          i +
          '" min="1" max="' +
          question.boxes.length +
          '"></td></tr>';
      }
      html += "</tbody></table>";
    } else if (question.type == "choice") {
      ...
      html =
        question.question +
        "<br>" +
        '<input type="number" id="ageInput" min="' +
        question.min +
        '" max="' +
        question.max +
        '">';
      ...
      for (let i = 1; i < 10; i++) {
        html +=
          '<select id="ladderInput' +
          i +
          '"><option value="-1">Select a choice</option><option value="1">' +
          (10 - i) +
          '0% Chance chance of $1.00</option><option value="0">$0.50 for sure</option></select><br><br>';
      }
    }
    $.confirm({
      title: question.title,
            ...
            if (question.type == "textbox") {
              ...
              for (let i = 0; i < question.boxes.length; i++) {
                ...
                if (!boxval) {
                  ...
                  if (
                    parseInt(boxval, 10) < 1 ||
                    parseInt(boxval, 10) > question.boxes.length
                  ) {
                    error = true;
                  }
                  boxVals.push(boxval);
              ...
              if (!error) {
                ...
                $.alert({
                  ...
                  useBootstrap: false,
                  content:
                    "Please enter the numbers 1 through 5 in the boxes. You may only enter each number once.",
                  type: "red",
                });
                return false;
              }
              ...
              for (let i = 1; i < 10; i++) {
                ans =
                  ans &&
                  document.getElementById("ladderInput" + i).value != "-1";
              }
              if (!ans) {
                $.alert({
                  title: "Error",
                  ...
                  useBootstrap: false,
                  content:
                    "Please make sure you select answers for all choices.",
                  type: "red",
                });
                return false;
              } else {
                ...
                for (let i = 1; i < 10; i++) {
                  prob.push(
                    parseInt(document.getElementById("ladderInput" + i).value)
                  );
                }
                window.expData.postQuestions.push({
                  question: question.title,
...
function dataToCSV() {
  ...
  for (i = 0; i < window.expData.preQuestions.length; i++) {
    csv +=
      '"' +
      window.expData.preQuestions[i].question +
      '","' +
  ...
  if (window.expData.postQuestions.length == 0) {
    ...
    for (i = 0; i < window.expData.postQuestions.length; i++) {
      csv +=
        '"' +
        window.expData.postQuestions[i].question +
        '","' +
  ...
  for (let i = 0; i < window.expData.trialData.length; i++) {
    csv +=
      '"' +
      window.expData.trialData[i].block +
      '","' +
      window.expData.trialData[i].trial +
      '","' +
      window.expData.trialData[i].boxes +
      '","' +
      window.expData.trialData[i].max +
      '","[' +
      window.expData.trialData[i].order +
      ']","[' +
      window.expData.trialData[i].vals +
      ']","' +
      window.expData.trialData[i].set +
      '","[' +
      window.expData.trialData[i].random +
      ']"\n';
  }
...
// Function to draw the canvas for each box
function drawCanvas(boxDiv, boxes) {
  let v, box, nButton, nCanvas, nText;
  boxDiv.innerHTML = "";
  for (let i = 0; i < boxes.length; i++) {
    box =
      boxes[
        window.expData.randomOrder[window.blk][window.trialNumber].boxes[i]
      ];
...
function startTrial() {
  ...
  const boxes =
    window.expParam.boxes[window.blk].sets[
      window.expData.randomOrder[window.blk][window.trialNumber].set
    ];
  ...
  for (let i = 0; i < boxList.length; i++) {
    boxList[i].onclick = function () {
      if (
        ...
        !this.classList.contains("mutednew")
      ) {
...
function stopSearch() {
  ...
  if (window.blk < window.expParam.boxes.length) {
    ...
    $.confirm({
      ...
      content: `<strong>Amount Paid to Open Boxes: </strong>${
        window.boxNum * window.expParam.searchCost
      }<br><strong>Winnings: </strong>${
        window.maxPoint
      }<br><br> Click "NEXT" to continue to the next trial.`,
      type: "blue",
      boxWidth: "55%",
      useBootstrap: false,
      typeAnimated: true,
    ...
    $.confirm({
      ...
      content: `<strong>Amount Paid to Open Boxes: </strong>${
        window.boxNum * window.expParam.searchCost
      }<br><strong>Winnings: </strong>${window.maxPoint}`,
      type: "blue",
      boxWidth: "55%",
      useBootstrap: false,
      typeAnimated: true,
